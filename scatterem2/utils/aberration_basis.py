...
Written from the standard expansion. It supplies the aberration phase and the
polar/cartesian conversions used across the reconstruction code.
...
are non-negative by construction, and the reported pair describes the same
wavefront as the cartesian coefficients it came from -- which is the property
that makes a (magnitude, angle) pair meaningful at all.
"""
...
def cartesian_chi(
    ...
    Args:
        ...
        wavelength: electron wavelength in Angstrom (see
            :func:`scatterem2.utils.physics.electron_wavelength`).
